...
import com.kuanquan.ijk_demo.R;
...
public class PlayerManagerBase {
     ...
     * 状态常量
     */
    private final int STATUS_ERROR=-1;     // 错误
    private final int STATUS_IDLE=0;       // 空闲
    private final int STATUS_LOADING=1;    // 加载中
    private final int STATUS_PLAYING=2;    // 播放中
    private final int STATUS_PAUSE=3;      // 暂停
    private final int STATUS_COMPLETED=4;  // 播放完
    ...
    private boolean fullScreenOnly;   // 是否为全屏 （横屏）
    private boolean portrait;     // 垂直

    private final int mMaxVolume;     // 最大音量
    private int screenWidthPixels;    // 屏幕实际宽度
    ...
    private PlayerStateListener playerStateListener;          // 播放状态监听
    ...
    // 初始化
    public PlayerManagerBase(final Activity activity) {
        try {
...
//        }
    ...
    public PlayerManagerBase playInFullScreen(boolean fullScreen){
        if (fullScreen) {
            activity.setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE);
        }
        return this;
    ...
    public PlayerManagerBase onError(OnErrorListener onErrorListener) {
        this.onErrorListener = onErrorListener;
        return this;
    }

    public PlayerManagerBase onComplete(OnCompleteListener onCompleteListener) {
        this.onCompleteListener = onCompleteListener;
        return this;
    }

    public PlayerManagerBase onInfo(OnInfoListener onInfoListener) {
        this.onInfoListener = onInfoListener;
        return this;
    }

    public PlayerManagerBase onControlPanelVisibilityChange(OnControlPanelVisibilityChangeListener listener){
     ...
     * @return
     */
    public PlayerManagerBase live(boolean isLive) {
        this.isLive = isLive;
        return this;
    }

    public PlayerManagerBase toggleAspectRatio(){
        if (videoView != null) {
            videoView.toggleAspectRatio();
        }
        return this;
